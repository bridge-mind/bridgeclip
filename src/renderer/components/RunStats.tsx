...
import { formatTimes, OPUS_CLIP, opusClipCostUsd, opusClipCredits, percentLessThanOpusClip, timesFasterThanOpusClip } from '../config/opus-clip'
...
/**
 * The run at a glance: clips, processing time and API cost (each compared with
 * OpusClip when this run beat it) and when it was made.
 */
export function RunStats({ output, costs, videoSpeed }: RunStatsProps): React.JSX.Element {
  ...
  // Only a run with a complete cost, cheaper or faster than OpusClip, makes a claim.
  const exactCost = costs && !costs.cost_incomplete ? costs.total_estimated_cost_usd : null
  const percentLess = exactCost !== null ? percentLessThanOpusClip(exactCost, sourceSeconds) : null
  const theirCost = opusClipCostUsd(sourceSeconds)
  const faster = timesFasterThanOpusClip(output.processing_time_seconds, sourceSeconds)
  ...
  const processingCell = (
    <StatCell
      ...
      sub={faster ? <Win>{formatTimes(faster)} faster than OpusClip</Win> : <><Num>{formatTimecode(sourceSeconds * 1000)}</Num> source</>}
    />
  )
  const costCell = (
    <StatCell
      ...
      final={costs ? formatUsd(costs.total_estimated_cost_usd) : 'Not recorded'}
      sub={percentLess ? <Win>{percentLess}% less than OpusClip</Win> : costs ? costParts(costs) : 'Not recorded for this run'}
    />
  )
  ...
  return (
    <div role="group" aria-label="Run stats" className="glass grid grid-cols-2 overflow-hidden rounded-3xl min-[960px]:grid-cols-4 [&>*:nth-child(-n+2)]:border-b [&>*:nth-child(odd)]:border-r min-[960px]:[&>*:nth-child(-n+2)]:border-b-0 min-[960px]:[&>*:not(:last-child)]:border-r [&>*]:border-white/[0.06]">
      ...
      {faster ? (
        <HoverCard
          label={`Processing took ${formatDuration(processingMs)}, ${formatTimes(faster)} faster than OpusClip`}
          content={
            <OpusClipCard
              headline={`${formatTimes(faster)} faster`}
              basis={`OpusClip says processing typically takes ${OPUS_CLIP.processingTerms}. This run took ${formatDuration(processingMs)} for ${formatTimecode(sourceSeconds * 1000)} of video.`}
            />
          }
      ...
      {percentLess && theirCost !== null && exactCost !== null ? (
        <HoverCard
          label={`API cost ${formatUsd(exactCost)}, ${percentLess}% less than OpusClip`}
          content={
            <OpusClipCard
              headline={`${percentLess}% less`}
              basis={`${opusClipCredits(sourceSeconds)} OpusClip credits (${OPUS_CLIP.priceTerms}) cost ${formatUsd(theirCost)}. This run cost ${formatUsd(exactCost)}. Prices as of ${OPUS_CLIP.checked}.`}
            />
          }
          className={HOVER_CELL}
        >
...
function OpusClipCard({ headline, basis }: { headline: string; basis: string }): React.JSX.Element {
  return (
    <div className="w-[260px] max-w-[calc(100vw-16px)]">
      <div className="px-4 pb-3.5 pt-4">
        ...
        <p className="mt-1.5 text-sm text-ink-muted">than OpusClip</p>
      </div>
