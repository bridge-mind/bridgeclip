...
Windows/Linux FFmpeg uses the LGPL shared upstream build, with OpenH264 for CPU encoding. Its LGPL version and dependency set differ from the minimal macOS build. Official publication is blocked until a reviewed corresponding-source distribution, including the exact dependency sources and notices, has a pinned URL and SHA-256 in private publication configuration. A link to upstream build recipes alone is not that distribution.
